...
const INTENT_PATTERNS: [RegExp, string][] = [
  ...
  [
    /industr|sector|field|domain|healthcare|finance|logistics|real.estate|e.commerce/i,
    "industries",
  ],
...
export function findRelevantKnowledge(message: string): string {
  ...
  if (intentData.length > 0) {
    ...
    const entries = buildKnowledgeEntries().filter((e) =>
      files.some(
        (f) =>
          f.startsWith(e.source) || e.source.startsWith(f.replace(".md", "").replace(".json", "")),
      ),
    );
...
export function loadFallbackFAQ(): string {
  ...
  if (faqJson && Array.isArray(faqJson)) {
    return faqJson.map((item) => `Q: ${item.question}\nA: ${item.answer}`).join("\n\n");
  }
