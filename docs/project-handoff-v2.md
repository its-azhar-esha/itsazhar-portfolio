...
| 00006 | `00006_create_media_files.sql`               | Media library                              | `media` bucket (public) + `media_files` (filename, storage_path, public_url nullable, mime, size, width/height, alt_text, caption)                            | anon: read; authed: CRUD. Storage object policies (public read, authed upload/update/delete). 10 MB enforcements (raised to 50 MB by 00041) |
...
- **Upload flow:** `MediaUploader` → `validateMediaFile` (mime + 50 MB) →
  `uploadMediaFile` (`src/lib/media/upload.ts`) uploads via TUS resumable
  uploads (chunked PATCHes to `/storage/v1/upload/resumable`, 6 MB chunks,
  resume-after-drop) with the session token for real progress → dimensions
  extracted client-side → `storeMediaAction` inserts the `media_files` row
  (orphan object removed best-effort on failure)
...
- **Size limit:** 50 MB per file — the Supabase Free plan global maximum
  (bucket `media` limit raised from 10 MB by migration `00041`; verified live,
  `file_size_limit: 52428800`). 100 MB+ requires a Pro plan. Enforced in
  `validateMediaFile`, the `media_files` Zod schemas and the bucket itself.
...
- REST probes confirm: `services`/`seo_metadata`/`media_files` → 200;
  ...
  storage buckets `media` (50 MB since migration 00041, all mime types) + legacy `project-media`
  both exist
